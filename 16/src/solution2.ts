import fs = require('fs');
import path = require('path');
import * as _ from "lodash";
import { result } from 'lodash';
import { ChildProcess } from 'child_process';


const input = fs.readFileSync(
  path.resolve(__dirname, '../input/input1.txt'),
  'utf-8'
);
const lines = input.split('\n');
let  sum = 0;
let  calltree = [];
let id = 0;

const hex2bin = (hex) => {
  hex = hex.replace("0x", "").toLowerCase();
  var out = "";
  for(var c of hex) {
      switch(c) {
          case '0': out += "0000"; break;
          case '1': out += "0001"; break;
          case '2': out += "0010"; break;
          case '3': out += "0011"; break;
          case '4': out += "0100"; break;
          case '5': out += "0101"; break;
          case '6': out += "0110"; break;
          case '7': out += "0111"; break;
          case '8': out += "1000"; break;
          case '9': out += "1001"; break;
          case 'a': out += "1010"; break;
          case 'b': out += "1011"; break;
          case 'c': out += "1100"; break;
          case 'd': out += "1101"; break;
          case 'e': out += "1110"; break;
          case 'f': out += "1111"; break;
          default: return "";
      }
  }
  return out;
}

const getLiteral = (decodedLit, id ) => {
  let prefix = decodedLit.charAt(0);
  let counter = 0;
  let digitsStr = '';


  while (prefix == '1'){
    counter++;
    digitsStr = digitsStr + decodedLit.substr(counter, 4);
    counter = counter + 4;
    prefix = decodedLit.charAt(counter);
  }
  if (prefix == '0'){
    counter++;
    digitsStr = digitsStr + decodedLit.substr(counter, 4);
    counter = counter + 4;
  }
  console.log('bin:' + digitsStr + ' dec:' +  parseInt(digitsStr, 2) );

  _.filter(calltree, {'id': id })[0]['value'] = parseInt(digitsStr, 2);

  return counter;
}



const decodePackage = (bin, myParId) => {
  const version = bin.substring(0,3);
  const type = bin.substring(3,6);
  sum = sum + parseInt(version, 2)
  let literalBin = '';
  let literalDec = 0;

  id++;

  let myId = id; 


  calltree.push({'id': myId, 'parentId': myParId,  'type' : parseInt(type, 2), 'value': 0});

  if (type == '100') {
    //literal value
    let packagelength = getLiteral(bin.substring(6), id);
    return packagelength+6;
  }
  else {
    //subpackages
    const lengthType = bin.charAt(6);
    if (lengthType == '0') {
      //next 15 bits are a number that represents the total length
      const totalLength = bin.substr(7,15);
      const totalLengthDec = parseInt(totalLength, 2);
      let subpackages = bin.substr(22, totalLengthDec); //substring from pos 22 length=numberofbits
      let proccessedBin = 0;
      let myLenght = 22;
      let counter = 0;
      while (proccessedBin < totalLengthDec) {
        let subpackagelength = decodePackage(subpackages, myId);
        counter++;
        myLenght = myLenght +subpackagelength
        const currentPackage = subpackages.substr(0, subpackagelength);
        console.log('currentPackage:' + currentPackage );

        proccessedBin = proccessedBin + subpackagelength;
        
        subpackages = subpackages.substr(subpackagelength);
      }
      return myLenght;
    }
    if (lengthType == '1') {
      //next 11 bits are a number that represents the number of sub-packets
      const numberPackages = bin.substr(7,11); 
      const numberPackagesDec = parseInt(numberPackages, 2);
      let subpackages = bin.substr(18);
      let myLenght = 18;
      let counter = 0;

      for ( let i=1; i<= numberPackagesDec; i++){
        let subpackagelength = decodePackage(subpackages, myId);
        counter ++;
        myLenght = myLenght +subpackagelength
        const currentPackage = subpackages.substr(0,subpackagelength);
        console.log('currentPackage:' + currentPackage );
        subpackages = subpackages.substr(subpackagelength);
      }
      return myLenght;
    }

  }
  ;
}


const calcValue = (myId, myParId) => {
  let result = 0;
  let temp1 = 0;
  let temp2 = 0;


  let myValues =  _.filter(calltree, {'id': myId , 'parentId': myParId })[0];
  if (myValues['type'] == 4 ) {
    return myValues['value'];    
  }

  const childs = _.filter(calltree, {'parentId': myId });

  switch  ( myValues['type'] ){
    case 0:
        childs.forEach((child,i) => {
          result = result + calcValue(child['id'], myId);
        });
        return result;
      break;
    case 1:
      result = 1;
      childs.forEach((child,i) => {
        result = result * calcValue(child['id'], myId);
      });
      return result
      break;
    case 2:
      result = 9999999999999999999999;
      childs.forEach((child,i) => {
        let temp = calcValue(child['id'], myId);
        if (temp < result) {
          result = temp;
        };
      });
      return result
      break;
    case 3:
      childs.forEach((child,i) => {
        let temp = calcValue(child['id'], myId);
        if (temp > result) {
          result = temp;
        };
      });
      return result
      break;
    case 5:
        temp1 = calcValue(childs[0]['id'], myId)
        temp2 = calcValue(childs[1]['id'], myId)
        if (temp1>temp2) {
          return 1;
        }
        else {
          return 0;
        }
      break;
    case 6:
      temp1 = calcValue(childs[0]['id'], myId)
      temp2 = calcValue(childs[1]['id'], myId)
      if (temp2>temp1) {
        return 1;
      }
      else {
        return 0;
      }
      break;
    case 7: 
    temp1 = calcValue(childs[0]['id'], myId)
    temp2 = calcValue(childs[1]['id'], myId)
    if (temp1 == temp2) {
        return 1;
      }
      else {
        return 0;
      }
      break;
  }
  

  return result; 
}

export const solution2 = () => {
  const hex = '';
  const bin = hex2bin(hex);
  decodePackage(bin, 0 );
  const result = calcValue(1, 0);
  return result;
};
