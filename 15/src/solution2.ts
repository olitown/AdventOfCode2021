...
const initCostmap = (costmap)  =>  {
 
  
  const numLines = costmap.length;
  const numCol = costmap[0].length;
  const factor = 5;
  let tempCostmap = new Array(numLines*factor).fill(0).map(() => new Array(numCol*factor).fill(0) );

  for (let i = 0; i < factor; i++) {
    costmap.forEach((line, x) => {
        const newX = i * numLines + x;

        for (let j = 0; j < 5; j++) {
            line.forEach((cost, y) => {
                const newY = j * numCol + y;
                let realCost = cost + i + j;

                if (realCost > 9) {
                    realCost = realCost % 9;
                }
                tempCostmap[newX][newY] = realCost;
            });
        }
    });
}
...
export const solution2 = () => {
  let costmaps = lines.map((line) => line.split('').map(Number));
  costmaps = initCostmap(costmaps);
  checkCost(costmaps)
  return costmaps;
};